...
from utils import (
    ...
    get_all_viable_bandit_positions, update_bandit, get_settlements_adjacent_to_tile, update_discard_pile,
    get_sum_of_array
)
...
def build_road_free(repo: git.Repo, local_player: int, rp: Road_point):
    # update state
    update_road_point(repo, rp.index, repo.active_branch.name)
    update_player_buildings(repo, local_player, [-1, 0, 0])
...
def try_to_build(repo: git.Repo, resources: [int], local_player: int, settlement_points: [Settlement_point], viable_rp: [Road_point], viable_sp: [Settlement_point] ,tried: [str]):
    ...
    elif can_buy_dev_card(repo, resources) and randrange(0, 10) < 3:
...
def play_card(repo: git.Repo, playable_cards, local_player: int, viable_rp: [Road_point], hexagons: [HexagonTile]):
    ...
    if type == 0:
        ...
        for player in range(_number_of_players):
            if player != local_player:
                ...
                update_player_hand(repo, "resource_cards", player, negate_int_arr(diff))
    ...
    # "Road-Building"
        ...
        build_road_free(repo, local_player, rp)
        build_road_free(repo, local_player, rp2)
    ...
    if type < 3:
        diff = [0,0,0,0]
        diff[type] += 1
        update_discard_pile(repo, diff[0:3])
        update_player_hand(repo, "available_cards", local_player, negate_int_arr(diff))
        files.append(os.path.join(repo.working_dir, "state", "game", "discard_pile"))
    else:
        update_player_hand(repo, "available_cards", local_player, [0,0,0,-1])
        update_player_hand(repo, "unveiled_cards", local_player, [1,0])
        files.append(os.path.join(repo.working_dir, "state", "game", "player_hands", f"player_{local_player + 1}",
                                  "unveiled_cards"))
